...
//        System.out.println("添加水印文字完成!");
    ...
    public static void markImg(String path,String text){
        int width = 200;
        int heigth = 150;
        Font font = new Font("微软雅黑", Font.ROMAN_BASELINE, 33);//字体
        BufferedImage bi1 = waterMarkByText(width, heigth, text, Color.GRAY, font, -30d,
                0.2f);//给图片添加文字水印
        try {
            ImageIO.write(bi1, "png", new File(path));//写入文件
