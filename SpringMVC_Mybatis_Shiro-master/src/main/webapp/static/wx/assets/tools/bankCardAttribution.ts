...
export function bankCardAttribution(bankCard): CardInfo {
  let cardTypeMap = {
    DC: "储蓄卡",
    CC: "信用卡",
    SCC: "准贷记卡",
  ...
  var bankcardList = [{
    ...
    patterns: [{
      ...
      reg: /^(62215049|62215050|62215051|62218065|62218850|62218851|62218849)\d{11}$/g,
      cardType: "DC"
    }, {
    ...
    for (let i = 0, len = bankcardList.length; i < len; i++) {
      ...
      for (let j = 0, jLen = patterns.length; j < jLen; j++) {
        ...
        if ((new RegExp(pattern.reg)).test(cardNo)) {
          ...
          delete (info as any).patterns;
          delete (info as any).reg;
